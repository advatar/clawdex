...
Active:
- [x] 2026-04-01 openclaw docs cleanup: rebase the pre-existing OpenMind docs edits onto the current recorded `openclaw` head, commit them in the submodule, and bump the superproject pointer.
- [x] 2026-04-01 submodule sync run: update all configured submodules to their latest reachable upstream heads, bump pinned SHAs in this repo, initialize the missing `Packages/DeepThink` + `plugins/get-shit-done` checkouts, and preserve the pre-existing dirty `openclaw` worktree.
...
- [x] Keep submodule bump commits isolated (one `chore(submodules)` commit), and land parity/feature ports as separate commits with tests.
  ...
  - Latest bump: 2026-04-01 (codex `547f7071f`, openclaw `5443693fb4`, DeepThink `ca01ce4bd`, get-shit-done `b85247a3b`)
