    ...
    T: PackedCompareControl,
{
    ...
    unsafe fn cmpestrm(&self, haystack: &[u8], leading_junk: usize) -> Option<usize> {
        ...
        let mask = match T::CONTROL_BYTE {
            ...
            _SIDD_CMP_EQUAL_ORDERED => _mm_cmpestrm(
                self.0.needle(),
                self.0.needle_len(),
                haystack,
                BYTES_PER_OPERATION as i32,
            ...
            _ => unreachable!("unsupported PackedCompareControl CONTROL_BYTE"),
        };
    ...
    unsafe fn cmpestri(&self, haystack: *const u8, haystack_len: i32) -> Option<usize> {
        ...
        let location = match T::CONTROL_BYTE {
            0 => _mm_cmpestri(
                self.0.needle(),
                self.0.needle_len(),
                haystack,
            ...
            _SIDD_CMP_EQUAL_ORDERED => _mm_cmpestri(
                self.0.needle(),
                self.0.needle_len(),
                haystack,
                haystack_len,
